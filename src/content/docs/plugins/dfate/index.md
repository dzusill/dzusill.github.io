---
title: "dFate"
description: "dFate asks every player one question the first time they join — hardcore or normal — and then holds them to the answer. A hardcore player who dies loses the…"
---

**dFate** asks every player one question the first time they join — hardcore or normal — and then holds them to the answer. A hardcore player who dies loses the account for 24 hours. The choice is permanent; only an admin can undo it.
...
- 🔮 **One choice, on the first join** — a native dialog screen with two buttons. No chest GUI, no chat command to remember.
- 🔒 **Nobody skips it** — the escape key is disabled, the player is frozen and made invulnerable until they answer, and the screen is put back if they disconnect mid-question.
...
- 🧩 **PlaceholderAPI** — 8 placeholders for TAB, scoreboards and chat formats.
...
first join
  └─ has a stored mode?
     ...
     └─ no  → freeze · invulnerable · show the choice screen
              ...
              └─ HARDCORE → "are you certain?" → stored, released
                            └─ death → filters → announce → ban → kick
                                       └─ ban expires → STILL hardcore
```
