...
| `api-key` | `""` | Shared service key. Must equal the API's `MC_PLUGIN_API_KEY`. Empty ⇒ `/webtoken` and sync are disabled. |
| `tenant-slug` | `default` | Sent as the `X-Tenant-Slug` header; selects which website tenant this server maps to. |
| `cooldown-seconds` | `30` | Minimum seconds between `/webtoken` uses per player. A failed request clears the cooldown so they can retry. |
| `profile-sync.enabled` | `true` | Master switch for pushing LuckPerms rank on join and after `/webtoken`. |
...
| `profile-sync.min-interval-seconds` | `300` | Minimum seconds between **automatic** (join) pushes per player. `/webtoken` pushes ignore this. |
