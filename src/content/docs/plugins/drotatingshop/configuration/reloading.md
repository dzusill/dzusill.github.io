...
It does **not** touch [data.yml](/plugins/drotatingshop/configuration/data/) — the current rotation, open window, live stock and per-player purchases all survive a reload.
...
| New / removed items (`items.yml`, `additem`, `removeitem`, `seed`) | Eligible from the **next [rotation](/plugins/drotatingshop/features/rotations/)**. |
...
> Want the changes live right now? Run `/dshop reload` then `/dshop rotate` to start a fresh rotation (and a fresh [open window](/plugins/drotatingshop/features/opening-hours/)) immediately.
