...
|---|---|---|
| `/market` | `drotatingshop.use` | Open the rotating shop. The name is set by `shop.command` in [config.yml](/plugins/drotatingshop/configuration/config/) — `market` is the default. |
...
| `/dshop rotate` | `drotatingshop.admin` | Force a rotation now. |
| `/dshop reload` | `drotatingshop.admin` | Reload `config.yml` / `items.yml` / `messages.yml`. |
...
| `drotatingshop.admin` | op | Every `/dshop` subcommand (additem, removeitem, list, rotate, reload). |
