...
> **DAnnounce registers no PlaceholderAPI expansion.** There is no `%dannounce_…%`. The placeholders on this page are substituted by DAnnounce itself in announcement text, and PlaceholderAPI — if you have it — resolves everything else.
...
The one thing that cannot be rewritten this way is a click line's command: the run-command event is attached to the finished component **after** parsing, so no tag reaching the body — from your own text or from a placeholder — can change which command a player ends up running. See [Writing an Announcement](/plugins/dannounce/features/announcements/#the-click-line).
...
```
[DAnnounce] Could not deliver announcement chat-announcements/discord to Steve
```

## Not the same as the command's placeholders

`messages.yml` has its own, unrelated set — `%id%`, `%variant%`, `%count%`, `%timezone%` and friends, each valid only in specific keys. They are listed on [messages.yml](/plugins/dannounce/configuration/messages/#placeholders-per-key).
