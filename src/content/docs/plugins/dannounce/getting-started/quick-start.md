...
> **Quote the timestamp.** An unquoted `at: 2026-09-01T18:00:00` is parsed by YAML as a timestamp in **UTC**, not as a string, and DAnnounce refuses it rather than guessing. See [Known Limitations](/plugins/dannounce/limitations/#an-unquoted-at-timestamp-is-rejected).

`DAILY` and `WEEKLY` work the same way — see [Schedules](/plugins/dannounce/features/schedules/).
...
Both need `dannounce.use`, which defaults to everyone, and both persist across restarts. Give a staff broadcast account `dannounce.bypass-toggle` to keep receiving regardless.

Next: [Channels](/plugins/dannounce/features/channels/).
