...
1. **Has the player muted announcements?** — `/announcements`, unless they hold `dannounce.bypass-toggle`
2. **Is `dannounce.receive.<id>` denied?** — unset means yes, receive it
...
Steps 3 to 5 are the audience rules. Step 1 is [Player Toggles](/plugins/dannounce/features/player-toggles/); step 2 is a permission node.
...
A channel with `min-players: 2` and one player online fires nothing. That is the setting doing its job: it exists so an almost-empty server does not talk to itself. It is not a bug, and it is the single most common reason a freshly installed DAnnounce appears to do nothing on a test server.
...
> **The entry's own node wins, and the channel's is the fallback.** `dannounce.receive.discord`
> excludes somebody from that one entry; `dannounce.receive.chat-announcements` excludes them from
