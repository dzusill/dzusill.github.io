...
Both need `dannounce.use`, which defaults to everyone.
...
A console sender, or a player without `dannounce.use`, gets the usage lines instead.
...
- the MOTD — see [MOTD & Join Delivery](/plugins/dannounce/features/motd-and-join/#it-honours-the-announcement-mute).
...
```
dannounce.bypass-toggle    default: false
```
...
```
[DAnnounce] Ignoring invalid UUID in state.yml: not-a-uuid
```
...
A preference confirmation is worth one glance and not worth a line of chat history. Change it to `CHAT` or `BOTH` if you disagree — see [messages.yml](/plugins/dannounce/configuration/messages/).
