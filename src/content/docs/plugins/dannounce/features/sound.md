...
> `dannounce.bypass-toggle` does **not** override this. It keeps a staff account receiving announcements they have muted; it has no effect on the sound preference, which is checked separately at delivery.

See [Player Toggles](/plugins/dannounce/features/player-toggles/).

## Command feedback sounds

The `sound:` block is announcement content. It has nothing to do with the `Presentation` block, which is how the command's own replies are delivered and can carry sounds of its own — see [messages.yml](/plugins/dannounce/configuration/messages/).
