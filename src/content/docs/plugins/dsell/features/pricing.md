...
See [prices.yml](/plugins/dsell/configuration/prices/) for the file itself, and
[Readable Item Keys](/plugins/dsell/features/item-keys/) for the key grammar.

> The `enchant-worth` block at the bottom of `prices.yml` came across with the migration and is **not read
> by the plugin**. Enchantment values live in [enchantments.yml](/plugins/dsell/configuration/enchantments/) and are
...
[making a migrated entry unsellable](/plugins/dsell/configuration/prices/#making-a-migrated-price-unsellable).
...
Rounded to `pricing.rounding`, floored at zero. See [Sell Multipliers](/plugins/dsell/features/sell-multipliers/) for the two
multiplier sources, and [Selling](/plugins/dsell/features/selling/) for what happens around the payout.
...
[`price-format` in config.yml](/plugins/dsell/configuration/config/#price-format) for grouping, decimals and the
