...
Needs `dsell.selltop`, granted to everyone by default. With nobody on the board yet the command
...
There are **no** sell-leaderboard placeholders yet. `%dsell_top_*%` is not implemented — the ranking
exists only as the `/selltop` menu. See [Placeholders](/plugins/dsell/placeholders/).

## Storage

The board is read from the YAML player store. There is no SQL backend, so it ranks the players on this
server only. See [`storage`](/plugins/dsell/configuration/config/#storage).
