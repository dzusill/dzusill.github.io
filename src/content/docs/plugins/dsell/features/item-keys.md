...
Every item has a **key** — the name it goes by in [prices.yml](/plugins/dsell/configuration/prices/), in
...
> This is a deliberate departure from the reference plugin DSell replaces, which stored prices as
...
The same grammar keys all three sections of [prices.yml](/plugins/dsell/configuration/prices/): your own `items`
overrides, the shipped `prices.by-serialized` potion variants, and the shipped `prices.by-material` table.
...
## Keys that name no item

Two kinds of key resolve to a price but cannot be drawn as an item, and both appear in the shipped table:

- a key naming a **material this server does not have** — a rename such as `CHAIN`, or a typo. Nothing can
  ever match it, so it is inert.
- a key naming a **block with no item form**, such as `BAMBOO_SAPLING` or `POTTED_CACTUS`. It prices
  normally, but there is no item to build an icon from, so it is left out of the `/worth` prices GUI.

Neither is an error and neither is pruned. See
[Migration health](/plugins/dsell/configuration/prices/#migration-health) for the counts.
...
[Enchantment Worth](/plugins/dsell/features/enchantment-worth/).
