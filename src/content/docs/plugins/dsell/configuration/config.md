...
dRotatingShop. That bridge is gone: [prices.yml](/plugins/dsell/configuration/prices/) is the only source of worth. The one thing
...
Money is held exactly and rounded once, at `rounding`. See [Pricing](/plugins/dsell/features/pricing/#money-is-exact).
...
[`/sellhistory`](/plugins/dsell/features/sell-history/), the `{amount}` and `{items}` tokens in the sold and summary
messages, and the item track of [`/selltop`](/plugins/dsell/features/sell-leaderboard/).
...
`%dsell_top_items_1%` follows this setting like every other count; the `_short` placeholder forms ask for an
abbreviation by name and stay compact regardless.

## Presentation

Where messages appear **and what they sound like**. Categories set the common case; `Overrides` changes one
exact [messages.yml key](/plugins/dsell/configuration/messages/), including silencing it completely.
...
individual message can be overridden, and `/dsell reload` applies changes immediately.
...
See [Worth Lore](/plugins/dsell/features/worth-lore/).
...
| `permission-based` | `true` | Honour `dsell.multiplier.<value>` nodes |
| `permission-cap` | `10.0` | Ceiling on the above, against typos. Floored at `1.0` |
| `reset.enabled` | `true` | Whether `/dsell resetmultiplier` may be used at all |
...
[sellaxe.yml](/plugins/dsell/configuration/sellaxe/).
...
`dsell.autosell`. See [Auto-Sell](/plugins/dsell/features/auto-sell/).
...
from it at once. See [Selling](/plugins/dsell/features/selling/#safety-properties).
...
[DSell] Item searches are typed into chat. Install ProtocolLib to type them into a sign instead.
```
...
See [GUIs](/plugins/dsell/configuration/guis/) for the icon and its wording.
...
**sqlite** keeps one database file, `dsell.db`, in the plugin folder. Nothing to install — the driver
...
`/dsell migrate playerdata` moves records into whichever backend is live, in either direction, so
...
[Custom Items](/plugins/dsell/features/custom-items/) for the full explanation of why this is config rather than
