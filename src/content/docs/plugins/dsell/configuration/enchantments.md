...
[Enchantment Worth](/plugins/dsell/features/enchantment-worth/) for how and when that addition happens; this page is
the file itself.
...
> [prices.yml](/plugins/dsell/configuration/prices/#the-enchant-worth-block) came across with the price migration and is **not** read.
...
3. the price of `ENCHANTED_BOOK[<name>=<level>]` in [prices.yml](/plugins/dsell/configuration/prices/)
...
be charged for twice. See [Enchantment Worth](/plugins/dsell/features/enchantment-worth/#enchanted-books).
...
`/dsell reload` picks up every change here. See [Reloading](/plugins/dsell/configuration/reloading/).
