...
`sellaxe.enabled` in [config.yml](/plugins/dsell/configuration/config/).
...
It works the same way [worth lore](/plugins/dsell/features/worth-lore/) does: the countdown is rewritten on the copy
...
See [The Sell Axe](/plugins/dsell/features/the-sell-axe/) for how the timer is stored and what it interacts with.
