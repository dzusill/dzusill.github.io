...
`sound` names an entry in [sounds.yml](/plugins/dsell/configuration/sounds/). `customModelData: -1` means none.
...
Sort labels come from [messages.yml](/plugins/dsell/configuration/messages/) (`sort_worth.*`). Filter labels are the categories' own
...
Whether the prompt is a sign or the chat box is [`search.input` in config.yml](/plugins/dsell/configuration/config/#search). Both
...
See [Sell History](/plugins/dsell/features/sell-history/).

---

## gui/multipliers.yml — multiplier overview

The category icons live in each [`sell/<id>.yml`](/plugins/dsell/configuration/sell-categories/) — their slot, material, title and
...
See [Sell Leaderboard](/plugins/dsell/features/sell-leaderboard/).
