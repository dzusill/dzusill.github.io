...
`/dsell reload` re-reads YAML. It does **not** load a new `DSell.jar` — Java has the old classes
...
you copy the block from the [GUIs page](/plugins/dsell/configuration/guis/) if you want to change it. Likewise a changed **default** for
...
Restart for: any setting in [the list above](#settings-that-need-a-restart), a new `DSell.jar`, or
