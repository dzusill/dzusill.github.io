...
[Pricing](/plugins/dsell/features/pricing/).
...
See [prices.yml](/plugins/dsell/configuration/prices/#making-a-migrated-price-unsellable).
...
[Migration health](/plugins/dsell/configuration/prices/#migration-health).
...
`scale` in [enchantments.yml](/plugins/dsell/configuration/enchantments/).
...
See [`price-format`](/plugins/dsell/configuration/config/#price-format).
...
[`Presentation`](/plugins/dsell/configuration/config/#presentation).
...
`playerdata.imported.yml`, so a second start cannot import it twice. `/dsell migrate playerdata` moves
...
Yes — `%dsell_top_*%` and the paging forms. They follow the same grammar as the sibling stats plugin, so the track
comes last: `%dsell_top_name_1_money%`, `%dsell_top_value_short_3_items%`,
`%dsell_top_list_1_10_money%`. See [Placeholders](/plugins/dsell/placeholders/).
...
The data folder is `plugins/DSell`. If you upgraded from **dDonutWorth** or **dDonutWorth**, rename the
old folder before the first start; otherwise a fresh config set is written and the old records are simply
not read. Permissions (`dsell.*`) and placeholders (`dsell_*`) changed with the name too.
...
DzusillCore and a Vault economy, always. **ProtocolLib is required too, but only for worth lore** — without
