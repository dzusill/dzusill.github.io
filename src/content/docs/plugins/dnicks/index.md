---
title: "dNicks"
description: "dNicks gives players colored and gradient nicknames that show up everywhere their name appears — in chat, in the tab list, in /msg, in join / quit / death…"
---

**dNicks** gives players **colored and gradient nicknames** that show up *everywhere* their name appears — in chat, in the tab list, in `/msg`, in join / quit / death messages, and (via `%dnicks_name%`) on the floating nametag above their head. Pick a style from an in-game GUI or type a full [MiniMessage](https://docs.papermc.io/adventure/minimessage/) nick by hand.
...
- 🏷️ **Gradient nametag, your way** — exposes `%dnicks_name%` so your nametag plugin (TAB, etc.) shows the gradient name above the head while keeping your rank prefix. One owner per surface, no conflicts.
...
- 🔌 **Plays nice** — exposes `%dnicks_name%` for PlaceholderAPI, wins chat over EssentialsX, and cooperates with TAB (roles stay yours, the name becomes the nick).
...
- [The Nametag Above the Head](/plugins/dnicks/features/nametag/)
