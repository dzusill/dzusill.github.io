...
`bossbar-announcements` is read so that an existing config still loads, and its keys are preserved verbatim on update, but nothing under it is ever turned into an announcement and no boss bar is ever shown. See [Known Limitations](/plugins/oberonannounce/limitations/#boss-bar-output-is-not-implemented).
