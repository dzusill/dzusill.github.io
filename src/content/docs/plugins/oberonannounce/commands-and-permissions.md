...
description: "Main command: /announcements. Built-in aliases /oberonannounce and /oa, plus whatever commands.aliases adds — /announce out of the box."
---

Main command: `/announcements`. Built-in aliases `/oberonannounce` and `/oa`, plus whatever `commands.aliases` adds — `/announce` out of the box.

| Command | Permission | Description |
|---|---|---|
| `/announcements` | `oberonannounce.use` | Toggle your own announcements. Player only; anyone else gets the usage lines |
| `/announcements toggle [messages\|sound]` | `oberonannounce.use` | The same, spelled out. `sound` toggles only the sound. Player only |
| `/announcements list` | `oberonannounce.admin` | Every announcement, its mode and state, with the entry ids under each |
| `/announcements next [id]` | `oberonannounce.admin` | When the next fire is — overall, or for one announcement |
| `/announcements preview <id> [variant]` | `oberonannounce.admin` | Render it to yourself, ignoring every rule. Player only |
| `/announcements send <id> [player]` | `oberonannounce.admin` | Send it for real, to everyone eligible or to one player |
| `/announcements reload` | `oberonannounce.admin` | Re-read `config.yml` and `messages.yml` |
...
Sends it to you alone and ignores everything: the audience rules, your own mute, `oberonannounce.receive.<id>`, `min-players`. It is the fastest way to check MiniMessage, placeholders and a click line without waiting for the interval.
...
Counting announcements, not entries. A refused reload keeps the running configuration — see [Reloading](/plugins/oberonannounce/configuration/reloading/).

## Permissions

| Node | Default | Grants |
|---|---|---|
| `oberonannounce.use` | `true` | `/announcements` and `/announcements toggle` |
| `oberonannounce.admin` | op | `list`, `next`, `preview`, `send`, `reload` |
| `oberonannounce.receive.<id>` | — | Receiving one announcement. See below |
| `oberonannounce.bypass-toggle` | `false` | Keep receiving announcements while they are toggled off |

`oberonannounce.use` defaults to everyone on purpose: opting out is the one thing a player can do here, and locking it would leave them no way to stop the messages.

### `oberonannounce.receive.<id>` is an opt-out
...
`<id>` is the announcement id, lower-cased. For a channel that is the channel — `oberonannounce.receive.chat-announcements` covers everything inside it, and there is no per-entry node. Announcements in the `announcements:` section each have their own.

`plugin.yml` declares the parent `oberonannounce.receive.*` with `default: true` so the node is discoverable in a permission plugin; it grants nothing on its own.
...
Added on top of `/announcements`, `/oberonannounce` and `/oa`. Entries are trimmed and lower-cased; blanks are skipped; an empty list falls back to `announce` rather than registering nothing.

> **A new alias needs a restart, not a reload.** Bukkit builds its command map when the plugin enables. See [Known Limitations](/plugins/oberonannounce/limitations/#command-aliases-need-a-restart).
