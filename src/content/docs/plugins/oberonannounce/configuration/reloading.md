---
title: "Reloading"
description: "Requires oberonannounce.admin. In one pass it:"
---
...
Requires `oberonannounce.admin`. In one pass it:
...
|---|---|---|
| Command aliases | Bukkit builds its command map once, at enable | Restart the server — see [Known Limitations](/plugins/oberonannounce/limitations/#command-aliases-need-a-restart) |
...
| The PlaceholderAPI hook | Registered at enable | Restart if PlaceholderAPI was installed after OberonAnnounce started |
...
- orphans its `oberonannounce.receive.<old-id>` permission node,
