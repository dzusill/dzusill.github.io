...
The bundled `config.yml` is the live Oberon SMP configuration: eleven chat announcements (`discord`, `info`, `reports`, `store`, `settings`, `apply`, `media`, `duels`, `bounty`, `coinflip`, `invest`), each with a title, one body line and a clickable command. The rotation fires every 900 seconds at random above two players, with `BLOCK_NOTE_BLOCK_CHIME` as the global sound and the MOTD switched off.
