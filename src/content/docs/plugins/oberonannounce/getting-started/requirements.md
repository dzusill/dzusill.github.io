---
title: "Requirements"
description: "No database, no economy, no Vault. The only file OberonAnnounce writes outside its config folder is nothing at all — its state lives in…"
---
...
| OberonCore (DzusillCore) | **1.12.0+** | The framework OberonAnnounce is built on — a separate jar in `plugins/`, declared as a hard `depend` |
...
No database, no economy, no Vault. The only file OberonAnnounce writes outside its config folder is nothing at all — its state lives in `plugins/OberonAnnounce/state.yml`.

## PlaceholderAPI

The hook is lazy: OberonAnnounce registers it at enable and uses it only if PlaceholderAPI is actually installed. Without it, announcements still work — the plugin's own `%player%`, `%online%`, `%max_players%`, `%announcement%` and `%variant%` are always substituted, and any `%…%` PlaceholderAPI would have resolved is left as literal text. See [Placeholders](/plugins/oberonannounce/placeholders/).
