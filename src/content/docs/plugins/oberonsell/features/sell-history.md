...
Viewing someone else's needs `oberonsell.history.others`.
...
[config.yml → Item counts](/plugins/oberonsell/configuration/config/#item-counts).

## Lifetime totals

Everything a player has ever earned from selling is tracked separately and exposed as
`%oberonsell_sold_total%`. See [Placeholders](/plugins/oberonsell/placeholders/).
