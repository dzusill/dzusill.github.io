...
See [prices.yml](/plugins/oberonsell/configuration/prices/#making-a-migrated-price-unsellable) for why.

This works on custom items too — hold an MMOItems sword and it becomes `mmoitems:SWORD/CUTLASS`. See
[Custom Items](/plugins/oberonsell/features/custom-items/).
...
[The Sell Axe](/plugins/oberonsell/features/the-sell-axe/).
...
Tune the ladders in `sell/<category>.yml` — see [Sell Multipliers](/plugins/oberonsell/features/sell-multipliers/).

## Where to go next

- [Pricing](/plugins/oberonsell/features/pricing/) — where a price comes from and how money is rounded
- [config.yml](/plugins/oberonsell/configuration/config/) — every key
