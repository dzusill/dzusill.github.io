...
See [Readable Item Keys](/plugins/oberonsell/features/item-keys/) for the full grammar. In short:
...
Mending on a tool, for any enchantment [enchantments.yml](/plugins/oberonsell/configuration/enchantments/) does not list.
...
> `/oberonsell doctor` reports the same figures on demand, and separates "explicitly unsellable" (a
...
values come from [enchantments.yml](/plugins/oberonsell/configuration/enchantments/), which has its own shape (`per-level` and exact
