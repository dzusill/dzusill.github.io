...
If the head is missing, see [The Donor's Head in Chat](/plugins/ddonations/features/player-heads/#troubleshooting) — the short version is that testing with your own name only shows a real head if you are online right now, or if your server is online-mode; a name it has never seen on an offline-mode backend needs a real UUID instead of a name.
...
- [Announcements](/plugins/ddonations/features/announcements/) — the eight channels and how to customise them per event
