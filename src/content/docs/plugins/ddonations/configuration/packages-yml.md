...
`{product}` and `{packages}` in `announcements.yml`, the Discord embed, `%ddonations_last_product%`, and the console.
...
`/donations packages` lists these by their readable name and marks them `(styled in store)`; `/donations packages stubs` writes **both** spellings into `match:`, because [community goals](/plugins/ddonations/configuration/goals-yml/) count in SQL against the exact stored name and would otherwise quietly count nothing.
...
- [announcements.yml](/plugins/ddonations/configuration/announcements-yml/)
