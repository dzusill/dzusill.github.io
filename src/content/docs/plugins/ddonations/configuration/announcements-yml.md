...
The multi-channel presentation rig — see [Announcements](/plugins/ddonations/features/announcements/) for how the pieces fit together conceptually. This page is the key-by-key reference.
...
| `player-head-style` | `pixels` (8×8 face art) or `icon` (native one-character head) — see [The Donor's Head in Chat](/plugins/ddonations/features/player-heads/) |
...
| `debug-chat-json` | Logs every rendered chat line as raw JSON, tagged `[chat-json]` — the diagnostic tool for a missing head; see [Troubleshooting](/plugins/ddonations/features/player-heads/#troubleshooting) |
...
| Money | `{amount}` `{amount_plain}` `{amount_raw}` `{currency}` `{currency_symbol}` — see [Currencies](/plugins/ddonations/features/currencies/) |
...
`{player_head}` is documented separately in [The Donor's Head in Chat](/plugins/ddonations/features/player-heads/) — it can carry a real image, not just text, and only ever renders inside `chat`.

`{product}` / `{packages}` show whatever [packages.yml](/plugins/ddonations/configuration/packages-yml/) calls each package, falling back to the store's own name; `{package_icon}` is that package's icon on its own. A package may also override this whole event for itself — see [Package Display Names](/plugins/ddonations/features/package-display-names/).
...
The one purchase-line migration this plugin performs on its own — `{player}` → `{player_head}` on the exact legacy line, and only that line — is covered in the [FAQ](/plugins/ddonations/faq/#i-upgraded-and-the-purchase-chat-line-still-says-player-not-player_head). Every other new key merges in automatically on load; `events:` itself never does, so any event you have edited is never touched by an update.

## See also

- [Announcements](/plugins/ddonations/features/announcements/)
- [The Donor's Head in Chat](/plugins/ddonations/features/player-heads/)
- [Reloading](/plugins/ddonations/configuration/reloading/)
