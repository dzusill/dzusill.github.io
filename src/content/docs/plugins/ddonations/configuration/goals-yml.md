...
Community goals. See [Community Goals](/plugins/ddonations/features/community-goals/) for how progress is computed and what each option means — this page is the file laid out in full.
...
`packages:` accepts a [packages.yml](/plugins/ddonations/configuration/packages-yml/) key (`nebula`), that package's display name, or the raw store id or name — all four mean the same package. Writing the key is the one that survives renaming the package on the store.

## See also

- [Community Goals](/plugins/ddonations/features/community-goals/)
- [Discord Webhooks](/plugins/ddonations/features/discord-webhooks/#events) — `goal-milestone` / `goal-complete` embeds
