...
### Why does dDonations not deliver the package I set up in Tebex?
...
This depends entirely on which NPC plugin is installed and has only been verified end-to-end on Citizens — FancyNpcs and ZNPCsPlus may render native placeholder text instead in some configurations. See [Known Limitations](/plugins/ddonations/limitations/#real-gaps-being-honest-about-them). A board with `npc.type: head` (a player skull on an armour stand, no NPC plugin needed) always works, as does the default `TEXT_DISPLAY` renderer.
