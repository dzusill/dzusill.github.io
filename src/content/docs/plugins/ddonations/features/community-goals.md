...
`{goal_bar}` is 25 segments; `%ddonations_goal_<id>_bar_10%` (PlaceholderAPI) is the same bar at 10 segments.

## Announcing it

`goal-milestone` and `goal-complete` in `announcements.yml` control presentation. See [Announcements](/plugins/ddonations/features/announcements/).
...
`%ddonations_goal_<id>_<field>%` — `name`, `description`, `percent`, `current`, `target`, `remaining`, `bar`, `bar_10`, `complete`, `cycle`. See [Placeholders](/plugins/ddonations/placeholders/#community-goals).

## See also

- [Discord Webhooks](/plugins/ddonations/features/discord-webhooks/) — `goal-milestone` and `goal-complete` embeds
- [goals.yml reference](/plugins/ddonations/configuration/goals-yml/)
