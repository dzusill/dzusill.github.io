...
A package called `Nebula Rank (Monthly) - MyServer` on Tebex can read as a gold-gradient **⭐ Nebula Rank** in game, keep that name in Discord and on `%ddonations_last_product%`, and bring its own announcement and embed with it — without touching the store.
...
- **A name in game**, everywhere a package is named: chat, Discord, `%ddonations_last_product%`, the console.
...
The full key-by-key reference is [packages.yml](/plugins/ddonations/configuration/packages-yml/).
...
An `announcement` block may set any channel **except `commands`**. Running a console command per package is how a second delivery system gets built by accident, next to the store's own plugin, and hands the same rank out twice. A package changes how a purchase looks; delivering it stays Tebex's job, permanently — see [Purchase Tracking](/plugins/ddonations/features/purchase-tracking/).
...
- [packages.yml reference](/plugins/ddonations/configuration/packages-yml/)
- [Announcements](/plugins/ddonations/features/announcements/)
- [Community Goals](/plugins/ddonations/features/community-goals/) — writing a goal against a package key
