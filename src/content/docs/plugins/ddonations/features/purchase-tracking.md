...
description: "dDonations observes your store; it never delivers anything. Two ways purchases reach it, and they can both be on at once."
---

dDonations observes your store; it never delivers anything. Two ways purchases reach it, and they can both be on at once.
...
Tebex signs each delivery with `HMAC-SHA256(key = webhook secret, data = hex(SHA256(raw body)))` — a hash of a hash, not a plain HMAC over the body. This is easy to get wrong writing a compatible verifier from scratch; dDonations implements it exactly this way and rejects anything that does not match.
...
- [Currencies](/plugins/ddonations/features/currencies/) — each purchase keeps its own currency, nothing is converted
- [Announcements](/plugins/ddonations/features/announcements/) — what happens once a purchase is recorded
