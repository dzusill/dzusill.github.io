...
either way — `hologram.locked` and `hologram.open` in [messages.yml](/plugins/dsupplydrops/configuration/messages/).
...
Any tier in [tiers.yml](/plugins/dsupplydrops/configuration/tiers/) may override any of those keys under its own
