...
[messages.yml](/plugins/dsupplydrops/configuration/messages/).
...
Only players holding `dsupplydrops.notify` receive any of it. That node is deliberately separate
...
| Sound | The `announce`, `land` and `unlock` [aliases](/plugins/dsupplydrops/configuration/sounds/) |
...
| Nothing at all for a rank | Revoke `dsupplydrops.notify` |
