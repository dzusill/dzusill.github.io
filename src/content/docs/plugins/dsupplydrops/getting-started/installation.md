---
title: "Installation"
description: "DzusillCore is a separate plugin jar and must already be in plugins/. It is not shaded into this"
---
...
| Java | 21 or newer |
| Required dependency | **DzusillCore** |
| Optional dependencies | PlaceholderAPI, WorldGuard, FancyHolograms 2.4.0+ |

DzusillCore is a separate plugin jar and must already be in `plugins/`. It is not shaded into this
...
[region filtering](/plugins/dsupplydrops/features/regions/) for landing sites, and FancyHolograms adds a richer
[hologram backend](/plugins/dsupplydrops/features/holograms/). Each is picked up when present and ignored when not.

## Steps

1. Drop `dSupplyDrops.jar` into `plugins/`.
2. Start the server. The default configuration is written to `plugins/dSupplyDrops/` and a
