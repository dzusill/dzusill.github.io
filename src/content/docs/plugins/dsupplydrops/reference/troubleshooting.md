...
| `surface is WATER` | Expected on an ocean-heavy map — raise `max-attempts`, or use [drop zones](/plugins/dsupplydrops/features/scheduling-and-zones/) over land |
...
| `blocked by the region rules` | See [WorldGuard regions](/plugins/dsupplydrops/features/regions/); check the blacklist is not swallowing the whitelist |
...
3. Switch to [drop zones](/plugins/dsupplydrops/features/scheduling-and-zones/) around the areas people actually play
   in.

## Nobody sees the announcements

- The `dsupplydrops.notify` permission is what gates every announcement, the title and the boss
