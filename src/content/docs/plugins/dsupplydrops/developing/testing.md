...
| `dSupplyDropsEnableTest` | The plugin boots, every shipped config parses, commands register |
...
```bash
./testserver.sh setup    # Paper 26.2 + DzusillCore + the plugin, on port 25567
...
This is what covers what MockBukkit cannot: that the plugin enables against a real DzusillCore, that
the shipped YAML parses on a real server, that a crate block genuinely appears and disappears, and
that no listener throws.

## Adding a test

Lifecycle tests belong in `me.dzusill.dsupplydrops.drop` — `tick(long)` is package-private, and
