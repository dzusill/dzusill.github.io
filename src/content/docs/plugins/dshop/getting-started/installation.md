...
1. Drop **DzusillCore.jar** and **DShop.jar** into `plugins/`.
2. Start the server.
...
Four lines from `[DShop]`:

```
Storage ready (sqlite).
Loaded 11 categories, 82 products.
Registered the %dshop_...% placeholders.
...
On first run, into `plugins/DShop/`:
...
[config.yml](/plugins/dshop/configuration/config/#sections-that-are-never-merged).
...
**Keep DzusillCore in step.** It is a separate jar, so running an older core against a newer DShop is
