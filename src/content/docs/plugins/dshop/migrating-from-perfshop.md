...
description: "DShop reads PerfShop's shop files as they are. The config style, the field names, the per-character"
---

DShop reads PerfShop's shop files as they are. The config style, the field names, the per-character
...
Copy `plugins/PerfShop/shops/*.yml` and `plugins/PerfShop/gui/*.yml` into `plugins/DShop/`, then
`/adminshop reload`.

Everything DShop adds is optional and additive: an item with no `stock`, `limit`, `sale`, `dynamic`,
...
| **`take-command` items** | These land on a `command` currency, which cannot check a balance or refund. See [Currencies](/plugins/dshop/features/currencies/#command-currencies). |
...
`--apply`. A report is written to `plugins/DShop/import-report.txt` either way.
...
**The purchase log itself is not copied.** DShop keeps counters, not a transaction history — that is
