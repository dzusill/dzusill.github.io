...
| `stock`, `limit`, `sale`, `dynamic` | See [Stock & Limits](/plugins/dshop/features/stock-and-limits/) and [Pricing & Sales](/plugins/dshop/features/pricing-and-sales/). |
| `custom-item` | See [Custom Items](/plugins/dshop/features/custom-items/). |
| `contents` | A bundle's outputs. |
| `commands`, `run-as` | A perk's commands. |
| `variants` | See [Variants](/plugins/dshop/features/variants/). |
...
> be refunded.** See [Currencies](/plugins/dshop/features/currencies/#command-currencies) before using it.

## Naming a category

Two rules, both checked at load with a message naming the file:

- **No underscore.** The placeholder form is `%dshop_price_<shop>_<product>%`, split on the first
